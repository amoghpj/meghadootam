...
#+begin_src devanagari
  \section*{{\dn \dnnum 29}}
  \begin{enumerate}
  \item[{\dn viicik.soBastanitavihaga}] 
  \item[{\dn sre.nikA~caiiju.nAyAH}] 
  \item[{\dn sa.msarpantyAH}] 
  \item[{\dn sKalitasuBaga.m}] 
  \item[{\dn dar"sitAvartanABeH}] 
  \item[{\dn nirvinDyAyAH}] 
  \item[{\dn paTi Bava}] 
  \item[{\dn rasAByantaraH sa.mnipatya}] 
  \item[{\dn strii.nAmAdya.m}] 
  \item[{\dn prauayavacana.m}] 
  \item[{\dn viBramo}] 
  \item[{\dn hi priye"su}] 
  \end{enumerate}
#+end_src
